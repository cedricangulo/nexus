...
export type SprintFilters = {
	query: string;
	status: "ALL" | "ACTIVE" | "UPCOMING" | "COMPLETED" | "OVERDUE";
	startDate?: string;
	endDate?: string;
};
...
export const getFilteredSprints = cache(
	async (
		...
		if (filters.startDate) {
			...
			// Sprint must end after filter start date to be included
			filtered = filtered.filter(
				(sprint) => new Date(sprint.endDate) >= filterStart,
			);
		}

		if (filters.endDate) {
			const filterEnd = new Date(filters.endDate);
			// Sprint must start before filter end date to be included
			filtered = filtered.filter(
				(sprint) => new Date(sprint.startDate) <= filterEnd,
			);
		}

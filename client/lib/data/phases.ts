 ...
 */
export const getPhases = cache(async (explicitToken?: string): Promise<Phase[]> => {
  try {
 ...
 */
export const getPhasesWithDetails = cache(async (explicitToken?: string): Promise<PhaseDetail[]> => {
  try {
    const api = await getApiClient(explicitToken);
    ...
    const phasesWithDetails = await Promise.all(
      phases.map(async (phase: Phase) => {
        try {
          const detailResponse = await api.get<PhaseDetail>(API_ENDPOINTS.PHASES.GET(phase.id));
 ...
 * Accepts optional token for 'use cache' compatibility
 */
export const getPhaseById = cache(async (
  id: string,
 ...
 */
export const getPhaseHeader = cache(async (
  id: string,
  explicitToken?: string
): Promise<{
  id: string;
 ...
 */
export const getPhaseTasksList = cache(async (
  id: string,
  explicitToken?: string
): Promise<Task[]> => {
  try {
 ...
 */
export const getPhaseDeliverablesList = cache(async (
  id: string,
  explicitToken?: string
): Promise<Deliverable[]> => {
  try {
 ...
 */
export const getPhaseMeetingsList = cache(async (
  id: string,
  explicitToken?: string
): Promise<MeetingLog[]> => {
  try {
 ...
 */
export const getPhaseAnalytics = cache(async (
  id: string,
  explicitToken?: string
): Promise<{
  totalTasks: number;
  ...
  try {
    ...
    const [phaseData, meetingsData] = await Promise.all([
      ...
      api.get<{ meetingLogs?: MeetingLog[] }>(API_ENDPOINTS.MEETING_LOGS.BY_PHASE(id)),
    ]);

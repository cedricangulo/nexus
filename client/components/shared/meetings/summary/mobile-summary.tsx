...
import { getCoverageData, getMissingMeetingsData, getOnTimeData, getTotalMeetingsData } from "@/lib/data/meetings";
...
export const SUMMARY_ITEMS = [
  { label: "Total", Component: TotalMobile, colorClass: "text-info-foreground" },
  {
    label: "Coverage",
...
export async function MobileSummary({
  ...
  return (
    <Frame className="sm:hidden">
      ...
      <FramePanel className="divide grid grid-cols-4 divide-x divide-dashed text-center p-2">
        {children}
      </FramePanel>
    </Frame>
  );

...
import { getActiveSprintBreakdown } from "@/lib/data/analytics";

export async function TaskSummary({ token }: { token: string }) {
  cacheLife("minutes");

  const breakdown = await getActiveSprintBreakdown(token);
        ...
        <Tooltip>
          ...
          <TooltipContent className="p-3">
            <div className="grid grid-cols-2 gap-x-3 gap-y-2 [&_svg]:text-xs">
              ...
              <div className="flex items-center gap-2">
                <div className="size-2 shrink-0 rounded-full bg-chart-3" />
                <span>{breakdown.inProgressTasks} In Progress</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="size-2 shrink-0 rounded-full bg-chart-4" />
